...
- [ ] 1.1 Create `src/editor/searchKeys.ts` with `KeyModifier`,
...
- [ ] 1.2 Gate `Enter`, `Shift+Enter`, `Tab` and `Shift+Tab` on
      ...
      collapsed.
- [ ] 1.3 Add `matchesBinding(event, binding, mac)` to
...
- [ ] 1.4 Export the tooltip captions from `src/editor/searchKeys.ts` and
...
- [ ] 2.1 Change `obsidianSearchPanel` in `src/editor/searchPanel.ts` to take
...
- [ ] 2.2 Add a stub `pushScope` to `harness/mount.ts`: a `document` `keydown`
      ...
      deleted listeners.
- [ ] 2.3 Confirm early, before the table grows harder to change, that a
      claimed key is not also acted on by CodeMirror: add a case to
      `tests/search.spec.ts` pressing `Escape` with the focus in the document
      and assert the bar closes exactly once and the document keeps its
      selection.

## 3. The Obsidian side

- [ ] 3.1 Rename `GedcomView.openSearch` to `showSearch(replace = false)` in
      ...
      pass.
- [ ] 3.2 Implement the private scope pusher in `src/GedcomView.ts` —
...
- [ ] 3.3 Add `hotkeys?: (mac: boolean) => CommandHotkey[]` to `GedcomCommand`
...
- [ ] 4.1 Give each button in `src/editor/searchPanel.ts` an `aria-label` of
...
- [ ] 5.1 Cover the keys that answer from either focus in
...
- [ ] 5.2 Cover `Alt+Enter` and `Mod+Alt+Enter` from the document in
      `tests/search.spec.ts`, asserting every match is selected and that
      replace-all rewrites the document.
- [ ] 5.3 Cover the focus gate in `tests/search.spec.ts`: with the bar open and
      the focus in the document, `Enter` inserts a line break without searching
      or closing, and `Tab` indents.
- [ ] 5.4 Cover field navigation in `tests/search.spec.ts`: `Tab` and
      `Shift+Tab` move between the find and replace fields with the replace row
      expanded, and do nothing while it is collapsed.
- [ ] 5.5 Cover the closed bar in `tests/search.spec.ts`: after the bar closes,
      `F3` no longer moves the selection — the scope was popped.

## 6. Verification

- [ ] 6.1 Run `npm run check` and `npm run test:browser`, and confirm both pass
